...
function getStore() {
  return JSON.parse(window.localStorage.getItem("places-ga") || "{}");
}

function setStore(places) {
  window.localStorage.setItem("places-ga", JSON.stringify(places));
}
...
App.prototype.displayScore = function () {
  var score = Object.keys(
    JSON.parse(window.localStorage.getItem("places-ga") || "{}")
  ).length;
  document.getElementById("score").innerHTML = score;
  document.getElementById("total").innerHTML = this.placeList.length;

  const shareText = `I just played Trackle™ and named ${score} GreaterAnglia stations! You can play Trackle™ too:`;
  const shareData = {
    title: "Trackle™",
    text: shareText,
...
App.prototype.resetGame = function () {
  // Clear local storage
  window.localStorage.removeItem("places-ga");

...
function getStore() {
  return JSON.parse(window.localStorage.getItem("places-lidle") || "{}"); // Retrieve and parse the "places-overground" object from local storage or return an empty object if it doesn't exist.
}

function setStore(places) {
  window.localStorage.setItem("places-lidle", JSON.stringify(places)); // Convert `places` to a JSON string and store it in local storage under the key "places-overground".
}
...
App.prototype.displayScore = function () {
  var score = Object.keys(
    JSON.parse(window.localStorage.getItem("places-lidle") || "{}")
...
App.prototype.winMessage = function () {
  alert("Well done! You found all " + this.placeList.length + " Lidle stores. April Fools 2026"); // Show an alert when the user wins.
};
...
App.prototype.resetGame = function () {
  ...
  window.localStorage.removeItem("places-lidle"); // Clear local storage.

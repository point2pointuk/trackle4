...
function getStore() {
  return JSON.parse(window.localStorage.getItem("places-c2c") || "{}");
}

function setStore(places) {
  window.localStorage.setItem("places-c2c", JSON.stringify(places));
}
...
App.prototype.displayScore = function () {
  var score = Object.keys(
    JSON.parse(window.localStorage.getItem("places-c2c") || "{}")
  ).length;
  document.getElementById("score").innerHTML = score;
  document.getElementById("total").innerHTML = this.placeList.length;

  const shareText = `I just played Trackle™ and named ${score} c2c stations! You can play Trackle™ too:`;
  const shareData = {
    title: "Trackle™",
    text: shareText,
...
App.prototype.resetGame = function () {
  // Clear local storage
  window.localStorage.removeItem("places-c2c");

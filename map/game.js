...
function getStore() {
  return JSON.parse(window.localStorage.getItem("places-map") || "{}");
}

function setStore(places) {
  window.localStorage.setItem("places-map", JSON.stringify(places));
}
...
App.prototype.displayScore = function () {
  var score = Object.keys(
    JSON.parse(window.localStorage.getItem("places-map") || "{}")
  ).length;
...
App.prototype.resetGame = function () {
  // Clear local storage
  window.localStorage.removeItem("places-map");

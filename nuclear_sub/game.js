...
function getStore() {
  return JSON.parse(window.localStorage.getItem("places-nucsub25") || "{}");
}

function setStore(places) {
  window.localStorage.setItem("places-nucsub25", JSON.stringify(places));
}
...
App.prototype.displayScore = function () {
  var score = Object.keys(
    JSON.parse(window.localStorage.getItem("places-nucsub25") || "{}")
  ).length;
...
App.prototype.resetGame = function () {
  // Clear local storage
  window.localStorage.removeItem("places-nucsub25");
